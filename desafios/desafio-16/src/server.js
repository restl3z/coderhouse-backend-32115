...
import { logger_console, logger_warn_file } from './logger.js';
...
const server = (PORT) => {
  ...
  app.get('*', (req, res) => {
    logger_console.warn(`${req.method} ${req.originalUrl} not available`);
    logger_warn_file.warn(`${req.method} ${req.originalUrl} not available`);
  });

  app.listen(PORT, () => {
    logger_console.info(`Server listening on port ${PORT}`);
  });
};
